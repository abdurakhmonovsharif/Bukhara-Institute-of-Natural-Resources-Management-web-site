...
const EditVideos = () => {
  ...
  return (
    <div className="videos-container">
      <h6>Videolar</h6>
      <button className="add-video-btn" onClick={() => setAddVideo(true)}>
        + Video qo'shish
      </button>
        ...
        {data?.map((item) => (
          <div key={item.id}>
            ...
            <div style={{ display: "flex", gap: "10px", marginBottom: "7px" }}>
              <button onClick={() => deleteVideos(item.id)} className="btn btn-danger">o'chirish</button>
              <button onClick={() => editVideos(item)} className="btn btn-primary">tahrirlash</button>
            </div>
          </div>
        ))}
      </Carousel>
