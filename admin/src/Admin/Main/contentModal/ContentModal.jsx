...
const ContentModal = () => {
  ...
  function convertTableToFlex(content) {
    ...
    if (tables.length > 0) {
      tables.forEach(table => {
        ...
        const firstCol = table.querySelector('[data-table-flex-first]')?.innerHTML
        const secondCol = table.querySelector('[data-table-flex-second]')?.innerHTML
        const thirdCol = table.querySelector('[data-table-flex-third]')?.innerHTML
        let flexChilds;
        if (flexesMeasure === '6') {
          flexChilds = `
  ...
  function convertFlexToTable(content) {
    ...
    if (flexes.length > 0) {
      flexes.forEach(flex => {
        ...
        const firstCol = flex.querySelector('[data-table-flex-first]')?.innerHTML
        const secondCol = flex.querySelector('[data-table-flex-second]')?.innerHTML
        const thirdCol = flex.querySelector('[data-table-flex-third]')?.innerHTML
        let tableChilds;
        if (flexesMeasure === '6') {
          tableChilds = `
            <tr>
  ...
  const handleSave = () => {
    if (method === 'add') {
      AxiosCall("post", `/content/categories${categoryIndex}/${id}`, {
        ...
        contents: [
          {
            ckEditor: { lan: "UZ", title: localStorage.getItem("uz_title"), body: convertTableToFlex(localStorage.getItem("uz_content")) },
          },
          {
            ckEditor: { lan: "RU", title: localStorage.getItem("ru_title"), body: convertTableToFlex(localStorage.getItem("ru_content")) },
          },
          {
            ckEditor: { lan: "EN", title: localStorage.getItem("en_title"), body: convertTableToFlex(localStorage.getItem("en_content")) },
          }
        ],
        urls,
        deletedUrls
  ...
  return (
    <div className="wrapper">
          ...
          <label className="image-uploader-label">
            ...
            <button type="button" className="button-image-upload">
              ...
              <span className="button__icon">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M21 15V18H24V20H21V23H19V20H16V18H19V15H21ZM21.0082 3C21.556 3 22 3.44495 22 3.9934V13H20V5H4V18.999L14 9L17 12V14.829L14 11.8284L6.827 19H14V21H2.9918C2.44405 21 2 20.5551 2 20.0066V3.9934C2 3.44476 2.45531 3 2.9918 3H21.0082ZM8 7C9.10457 7 10 7.89543 10 9C10 10.1046 9.10457 11 8 11C6.89543 11 6 10.1046 6 9C6 7.89543 6.89543 7 8 7Z" fill="rgba(250,250,250,1)"></path></svg>
              </span>
            </button>
          </label>
          <select onChange={(e) => setLanState(e.target.value)} value={lanState}>
            <option value="uz">UZB</option>
            <option value="ru">RU</option>
            <option value="en">ENG</option>
          </select>
          <button onClick={() => setAlertVisible(true)} className="delete-content-btn-disabled">o'chirish</button>
          <button
      ...
      <div className="body">
        <textarea placeholder={lanState == "uz" ? "Sarlavha" : lanState == "ru" ? "Заголовок" : "Title"} onInput={(e) => addTitle(e.target.value)} value={title} />
        <div className="p-relative">
          <Editor
            ...
            init={{
              ...
              toolbar_sticky: true,
              plugins: ['table', 'image', 'media', 'autoresize', 'template', 'link', 'lists'],
              ...
              setup(editor) {
                editor.on("keydown", function (e) {
                  if ((e.keyCode == 8 || e.keyCode == 46) && window.tinymce.activeEditor.selection) {
                    var selectedNode = window.tinymce.activeEditor.selection.getNode();
                    if (selectedNode && selectedNode.firstElementChild?.nodeName == 'IMG') {
                      var imageSrc = selectedNode.firstElementChild.src;
                      const imageId = imageSrc?.split('/').at(-1)
                      if (imageId) {
                        imageId && imageId && AxiosCall('delete', `/content_images/${imageId}`)
            ...
            {images.map((item, index) => (
              <div className="banner_item1" key={index}>
                <div className="delete-btn">
                  <FontAwesomeIcon icon={faTrash} onClick={() => deleteCarousel(item)} />
                </div>
      ...
      <Rodal width={250} height={120} visible={alertVisible} onClose={() => setAlertVisible(false)}>
        <div className="alert-box">
          <span>O'chirilsinmi?</span>
          <div className="btns">
            <button className='no-btn' onClick={() => setAlertVisible(false)}>yo'q</button>
            <button className='yes-btn' onClick={() => deleteItem()}>ha</button>
          </div>
        </div>
      </Rodal>
    </div>
