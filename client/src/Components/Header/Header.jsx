            ...
            {navbar.map((item1, index1) => {
              return (
                <div className="list" key={index1}>
                  <ul>
                    <li>
                      <h6 onClick={() => checkCategory(item1.id, 1, item1.name)}>{item1.name}
                        <svg className="down-icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M11.9997 13.1714L16.9495 8.22168L18.3637 9.63589L11.9997 15.9999L5.63574 9.63589L7.04996 8.22168L11.9997 13.1714Z" fill="rgba(251,248,248,1)"></path></svg></h6>
                      <ul>
                        {item1.innerCategories.map((item2, index2) => {
                          return (
                            <li key={index2}>
                              <div onClick={() => checkCategory(item2.id, 1, item2.name)}>{item2.name}</div>
                              <ul>
                                {item2.innerCategories.map((item3, index3) => {
                                  return (
                                    <li key={index3}>
                                      <div onClick={() => checkCategory(item3.id, 1, item3.name)}>{item3.name}</div>
                                      <ul>
                                        {item3.innerCategories.map((item4, index4) => {
                                          return (
                                            <li key={index4}>
                                              <div onClick={() => checkCategory(item4.id, 1, item4.name)}>{item4.name}</div>
                                            </li>
                                          );
                                        })}
                                      </ul>
      ...
      <header>
        ...
        <nav>
                ...
                <div className="container">
                  <div className="my-lan-select">
                    <div className="lan-selected-item">
                      {lang == "uz" ? <>
                        <img src="https://cdn-icons-png.flaticon.com/512/197/197416.png" alt="" width={"20px"} />
                        Uzb
                      </> : lang == "en" ? <>
                        <img src="https://cdn-icons-png.flaticon.com/512/197/197374.png" alt="" width={"20px"} />
                        Eng
                      </> :
                        <>
                          <img src="https://cdn-icons-png.flaticon.com/512/197/197408.png" alt="" width={"20px"} />
                          Rus
                        </>
                      }</div>
          ...
          <div className="bottom-side">
            <div className="ul">
              {
                headerCategory.map((item1, index1) => <div className="li" key={index1} >
                  <div id="li" onClick={() => checkCategory(item1.id, 1, item1.name)}>{item1.name}
                    {item1.innerCategories.length > 0 && <svg width={"18px"} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M12 16L6 10H18L12 16Z" fill="rgba(0,0,0,1)"></path></svg>}
                  </div>
                  {item1.innerCategories.length > 0 &&
                    <div className="ul">
                      {item1.innerCategories?.map((item2, index2) => <div key={index2} className="li" ><div id="li" onClick={() => checkCategory(item2.id, 2, item2.name)}>{item2.name}
                        {item2.innerCategories.length > 0 &&
                          <svg width={"18px"} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M16 12L10 18V6L16 12Z"></path></svg>}
                      </div>
                        {item2.innerCategories.length > 0 &&
                          <div className="ul">
                            {item2.innerCategories?.map((item3, index3) => <div key={index3} className="li" ><div id="li" onClick={() => checkCategory(item3.id, 3, item3.name)}>{item3.name}
                              {item3.innerCategories.length > 0 &&
                                <svg width={"18px"} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M16 12L10 18V6L16 12Z"></path></svg>}
                            </div>
                              <div className="ul">
                                {item3.innerCategories?.map((item4, index4) => <div key={index4} className="li" ><div id="li" onClick={() => checkCategory(item4.id, 4, item4.name)}>{item4.name}</div></div>)}
                              </div>
                            </div>)}
                          </div>
                        }
                      ...
                      searched.length > 0 ?
                        <div className="search-list">
                          {
                            searched?.map((item) => {
                              return (
                                <span className="search-list-item" onClick={() => {
                                  setSearchValue("")
                                  navigate(`/${item.title.replace(/\s+/g, '-').replace(/\//g, '-')}/${item.id}`)
                                  setOpenSearchCard(false)
                        ...
                        searchLoading ?
                          ...
                          <span className="search-list-item text-white">
                            {
                              lang == "uz" ? "Hech qanday ma'lumot topilmadi." : lang == "en" ? 'No information found.' : "Информация не найдена."
                            }
