  ...
  const checkCategory = (id, num, name) => {
    AxiosCall("get", `/content/${lang}/${id}/1`).then(data => {
      if (data.contents.length == 1) {
        navigate(`/${name.replace(/\s+/g, '-').replace(/\//g, '-')}/${data.contents[0].id}`)
  ...
  return (
    <>
      ...
      <section className="sigleComponent">
          ...
          <>
            {name == "flag" ? (
              <div className="flag-container">
                <h1>{lang == "uz" ? flagInfo.nameUZ : lang == 'en' ? flagInfo.nameEN : flagInfo.nameRU}</h1>
                <div className="image-container">
                  <img src={flagInfo.image1} alt="flag" />
                </div>
                <p>{lang == "uz" ? flagInfo.contentUZ : lang == 'en' ? flagInfo.contentEN : flagInfo.contentRU}</p>
              </div>
            ) : name == "emblem" ? (
              <section className="emblem">
                <h1>{lang == "uz" ? emblemInfo.nameUZ : lang == 'en' ? emblemInfo.nameEN : emblemInfo.nameRU}</h1>
                <div className="images">
                  <div className="image-container">
                    <img src={emblemInfo.image1} alt="flag" />
                  </div>
                </div>
                <p>{lang == "uz" ? emblemInfo.contentUZ : lang == 'en' ? emblemInfo.contentEN : emblemInfo.contentRU}</p>

              </section>
            ) : (
              <div className="content-container">
                {
                  Object.keys(data).length == 0 ? <span className="content"><span className="loading-text">{
                    lang == "uz" ? "Sahifa yangilanmoqda..." : lang == "ru" ? "Страница обновляется..." : "The page is refreshing..."
                  }</span></span> :
                        ...
                        <div className="content">
                          ...
                          {data.url.length > 0 &&
                            <Carousel className="carousel">
                              {
                                data?.url?.map((images, index) => <div key={index}>
                                  <div className="banner_item">
                                    <img className="fake-image" src={BASE_URL + images} alt="" />
                          ...
                          <div className="comment-btn">
                            <button onClick={() => setModalVisible(true)}>
                              Izoh qoldirish
                              <svg width={"22px"} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M6.45455 19L2 22.5V4C2 3.44772 2.44772 3 3 3H21C21.5523 3 22 3.44772 22 4V18C22 18.5523 21.5523 19 21 19H6.45455ZM11 10H8V12H11V15H13V12H16V10H13V7H11V10Z" fill="rgba(255,255,255,1)"></path></svg>
                            </button>
                          </div>
                        </div>
                      </>
                  ...
                  <div className="cards1-container">
                    ...
                    }
                    {
                      tuzilma.map((item, index) => {
                        return (
                          <span key={index} onClick={() => checkCategory(item?.id, 2, item?.name)}>{item?.name}</span>
                        )
                      })
                    }
                  </div>
                  <div className="cards2-container">
                    <h6>{lang == "uz" ? 'Yangiliklar' : lang == "en" ? "News" : "Новости"}</h6>
                    <div className="cards">
                      {news.map((item, index) => {
                        ...
                        return (
                          <div key={index} className="my-card1"
                            onClick={() => navigate(`/${item.title.replace(/\s+/g, '-').replace(/\//g, '-')}/${item.id}`)}
                          >
        ...
        {
          commentItems.length > 0 ? <div className="comments-container">
            ...
            <AliceCarousel
              ...
              items={commentItems?.map((item, index) => <div key={index} className="comment-card">
                <div className="commment-card-container"
                  onClick={() => setCommentOpen({ ...commentItems, hide: true, item: item.text })}
                >
                  ...
                  <div className="client-info">
                    <div className="image">
                      <img src="https://w7.pngwing.com/pngs/753/432/png-transparent-user-profile-2018-in-sight-user-conference-expo-business-default-business-angle-service-people-thumbnail.png" alt="image" />
                    </div>
            ...
            />
          </div> : <div className="no-comment"></div>
        }

        <Modal show={commentOpen.hide} onHide={() => setCommentOpen({ ...commentItems, hide: false })}>
          <Modal.Body>
